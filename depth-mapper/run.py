...
Give it an image filename (inside ./images/) and it will:
    ...
    2. Clean the depth map  (bilateral filter + edge sharpening)
    3. Segment into RGBA layers  (k-means)
...
def build_layers(image_bgr: np.ndarray, depth_map: np.ndarray, num_layers: int | None, stem: str):
    print("Cleaning depth map (bilateral filter + edge sharpening)...")
    original_depth = depth_map
    cleaned_depth  = clean_depth_map(depth_map, image_bgr)
    normalised     = normalise_depth_range(cleaned_depth)

    if num_layers is None:
        num_layers = estimate_num_layers(normalised)
    ...
    save_depth_coloured(original_depth, os.path.join(out_dir, "depth_original.png"))
    save_depth_coloured(cleaned_depth,  os.path.join(out_dir, "depth_cleaned.png"))
    save_depth_bands(normalised, num_layers, os.path.join(out_dir, "depth_bands.png"))

    print(f"Segmenting into {num_layers} layers (k-means)...")
    layers = segment_layers(image_bgr, cleaned_depth, num_layers=num_layers)
...
def main():
    ...
    print(f"  layers/{stem}/depth_cleaned.png   — after bilateral filter + edge sharpening")
    print(f"  layers/{stem}/depth_bands.png     — depth map with layer boundary lines")
